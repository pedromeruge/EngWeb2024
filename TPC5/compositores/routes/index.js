    ...
    .then( respostaCompositores => {
      ...
      axios.get('http://localhost:3000/periodos')
        .then(respostaPeriodos => {
          ...
          const todosPeriodos = respostaPeriodos.data;
          const periodoToUpdate = todosPeriodos.find( p => p.id === req.body.periodo[0])

          if (periodoToUpdate) {
              periodoToUpdate.compositores.push([req.body.id,req.body.nome]) // acrescentar novo compositor à lista de compositores desse período
...
router.post('/compositores/edit/:id',function(req,res) {
  ...
  .then( respostaCompositores => {
      ...
      if (req.body.periodo != req.body.periodoInicial) { // se utilizador mudou de período, atualizar as respetivas listas dos períodos
          ...
          .then(respostaPeriodos => {
              ...
              if (novoPeriodo && previoPeriodo) {
                  novoPeriodo.compositores.push([req.body.id,req.body.nome]) // acrescentar novo compositor à lista de compositores desse período
...
router.post('/periodos/edit/:id', function(req, res) {
  ...
  axios.patch('http://localhost:3000/periodos/' + req.params.id, req.body)
    .then( resposta => {
        res.render("periodo",{data: resposta.data, date: d})
    })
    .catch( erro => {
        res.render("periodosError", {error: erro, date: d, message: "Erro updating periodos"})
    })
})
